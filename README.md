...
The main server file can be run using `node` or `nodemon`.
...
Additionally, to manage the database, use the provided scripts:

- **Drop Database and Clear Static Data:**
  ```bash
  node dropDB.js
  ```
...
Upon starting, the server will connect to MongoDB, set up all routes, and listen for incoming connections on the specified port (default is `3000`).

## Conclusion

This project demonstrates a comprehensive implementation of a real-time chat system using the MEAN stack. By leveraging Angular for the frontend, Node.js and Express for the backend, MongoDB for data management, and Socket.io for real-time communication, the system ensures a responsive and interactive user experience. The structured approach to version control, data management, and the addition of video support further enhance the project's maintainability and scalability.